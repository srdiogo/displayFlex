const isPar = (x) => {
  if (typeof(x) !== 'number') {return "O argumento não é um número"};
  return x % 2 === 0;
  }
console.log(isPar('2'));

const div = (x, y) => {
    if (typeof(x||y) !== 'number') {return "O argumento não é um número"};
...
const torta = (x, y) => {
    if (typeof(x||y) !== 'number') {return "O argumento não é um número"};
    for (n=1; n<=x; n++){
        if(n % y !== 0) console.log(n);
        else console.log("torta");
    }; 
};
torta(10,3);
